        ...
        for query, response, history, system in construct_example(examples):
            ...
            if query:
                ...
                for turn_idx, (source_ids, target_ids) in enumerate(template.encode_multiturn(
                    ...
                    if total_len > data_args.cutoff_len:
                        print(f"total_len ({total_len}) > data_args.cutoff_len ({data_args.cutoff_len})")
                        print(query)
            ...
            # Memorize Book Knowledge
            else:
                assert response
                if history:
                    continue
                source_ids = tokenizer.encode(text="医学知识：\n\n", add_special_tokens=False)
                target_ids = tokenizer.encode(text=response, add_special_tokens=False) + [tokenizer.eos_token_id]

                input_ids = source_ids + target_ids
                # also ignore the first 25 tokens in target_ids to stablize training
                num_ignore = min(len(target_ids), 25)
                labels = [IGNORE_INDEX] * len(source_ids) + [IGNORE_INDEX] * num_ignore + target_ids[num_ignore:]
