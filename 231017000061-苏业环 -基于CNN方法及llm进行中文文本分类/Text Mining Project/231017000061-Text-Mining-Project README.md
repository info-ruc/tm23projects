...
老师您好，我是231017000061-22计算机秋季班-苏业环。

本次大作业，我采用的是基于CNN，RNN深度学习方法及llm模型对中文文本进行分类。本项目基于TensorFlow在中文数据集上的简化实现，使用了字符级CNN和RNN对中文文本进行分类，达到了较好的效果。

本次作业学习了前辈的开发经验，在其开发的基础之上，添加了部分自己对CNN及RNN的理解，并完成了项目的再现，并对相关的卷积算法进行了多次调试，达到了试验及学习预期。

## 环境

- Python 2/3 (调试Python2环境)
- TensorFlow 1.3以上
- numpy
- scikit-learn
- scipy
...
使用THUCNews（一个高效的中文文本分类工具包，非常好用）的一个子集进行训练与测试，数据在[THUCTC：](http://thuctc.thunlp.org/)下载，并且本项目我在开发的过程中严格遵循数据提供方的开源协议！
