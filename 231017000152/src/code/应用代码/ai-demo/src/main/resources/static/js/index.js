...
layui.config({
	base : "js/"
}).use(['bodyTab','form','element','layer','jquery','validator'],function(){
	var form = layui.form,
		layer = layui.layer,
	...
	function getParentMenus(){
		$.ajax({
		      url: "sysMenu/findParentMenus",
		      type: "get",
		      async: false,
		      success: function(result) { //返回数据根据结果进行相应的处理  
		    	var parentId;
		        var parentMenuHtml='<ul class="layui-nav beg-layout-nav" lay-filter="topnavside">';
		        for(var i=0,l=result.length;i<l;i++){
		        	parentId = result[i].id;
		        	if(i==0){
		        		tab = layui.bodyTab({
		        		openTabNum : "50",  //最大可打开窗口数量
		        		url : "sysMenu/getSysMenuPerms/"+parentId //获取菜单json地址
		        		//url : "sys/menu" //获取菜单json地址
		        		});
		        		parentMenuHtml+='<li class="layui-nav-item layui-this" data-pid="'+parentId+'">';
		            }else{
		        		parentMenuHtml+='<li class="layui-nav-item" data-pid="'+parentId+'">';
		            }
		        	parentMenuHtml+='<a href="javascript:;" data-pid="'+parentId+'">';
		        	parentMenuHtml+='<i></i><cite>'+result[i].name+'</cite>';
		        	parentMenuHtml+='</a>';
		        	parentMenuHtml+='</li>';
		        }
		        $("#menu").html(parentMenuHtml);
		    }
		});
	}	
	getParentMenus();
	...
	element.on('nav(topnavside)',function (elem) {
		var topvalue = elem[0];
		var _parentId = $(topvalue).attr("data-pid");
		//alert(_parentId);
		tab = layui.bodyTab({
			openTabNum : "50",  //最大可打开窗口数量
			url : "sysMenu/getSysMenuPerms/"+_parentId //获取菜单json地址
			//url : "sys/menu" //获取菜单json地址
		});
		//渲染左侧菜单
		tab.render();
	...
	function lockPage(){
		layer.open({
			...
			content : '	<div class="admin-header-lock" id="lock-box">'+
							'<div class="admin-header-lock-img"><img src="res/images/face.jpg"/></div>'+
